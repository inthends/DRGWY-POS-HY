  ...
  onBarCodeRead = (result) => {
    ...
    this.setState(
      ...
      () => {
        ...
        NavigatorService.createOrder(ids, isML, mlType, mlScale)
          .then((res) => {
            NavigatorService.scanPay(result.data, res.out_trade_no)
              .then((resp) => {
                if (resp === 'need_query') {
                  this.needQuery(res);
                } else {
  ...
  needQuery(res) {
    ...
    if (count === 7) {
      this.showLoadingNumber =
        UDToast.showLoading('正在查询支付结果，请稍后...');
    }
    this.setState(
      {
        count: count - 1,
      ...
      () => {
        if (count > 0) {
          NavigatorService.scanPayQuery(res.out_trade_no)
            .then((query) => {
              if (query === 'SUCCESS') {
